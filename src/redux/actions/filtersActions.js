...
export const filteredEvents = ({startDate,endDate,producer}) =>{
    ...
    return async(dispatch)=>{
        let filteredEvents;
            ...
            if(producer ===null && startDate && endDate===null) { const response = await axios.get(`http://localhost:3001/events/filter?startDate=${startDate}`);
            ...
            const response = await axios.get(`http://localhost:3001/events/filter?producer=${producer}&startDate=${startDate}`);
            filteredEvents=response.data;}
